...
int recalcularCentro (int *Puntos, double *Centroides, int *PC, int *Sep, int N, int D, int C)
{
  ...
  #pragma omp parallel for private(j,k,x) shared(Puntos,Centroides,PC,Sep,N,D,C) reduction(|:r)
  for (i=0; i<C; i++)
  {
    double v[D]; 
    for (x=0; x<D; x++)
...
void PuntosToCentroides(int *Puntos, double *Centroides, int *PC, int *Sep, int N, int D, int C)
{
  ...
  #pragma omp parallel for private(j,k,m) shared(Puntos,Centroides,PC,Sep,N,D,C)
  for(i=0; i<N; i++)
  {
    double dist[C];
    for(j=0; j<C; j++)
